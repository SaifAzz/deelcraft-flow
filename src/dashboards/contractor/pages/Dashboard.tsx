...
import { affindaOcrService, type AffindaOcrResult } from "@/shared/services/affindaOcrService";
...
const ComplianceDocumentsView = () => {
    ...
    const [ocrResults, setOcrResults] = useState<Record<string, AffindaOcrResult['data']>>({});
    ...
    const handleFileUpload = async (docId: string, file: File) => {
        console.log('File upload started:', { docId, fileName: file.name, fileSize: file.size, fileType: file.type });

        // Mark as processing
        setProcessingDocs({ ...processingDocs, [docId]: true });
        setOcrErrors({ ...ocrErrors, [docId]: undefined });

        try {
            // For passport/ID documents, extract text using Affinda OCR
            if (docId === "passport") {
                const documentType = file.name.toLowerCase().includes('passport') ? 'passport' : 'national_id';
                console.log('Processing passport/ID document:', { documentType, fileName: file.name });
                const result = await affindaOcrService.extractDocumentData(file, documentType);
                console.log('OCR result:', result);

                if (result.success && result.data) {
                    setOcrResults({ ...ocrResults, [docId]: result.data });
                    setUploadedDocs({ ...uploadedDocs, [docId]: true });
                    toast({
                        title: "Document processed successfully",
                        description: `Extracted data from ${documentType === 'passport' ? 'passport' : 'ID'}`,
                    });
                } else {
                    setOcrErrors({ ...ocrErrors, [docId]: result.error || 'Failed to extract document data' });
                    toast({
                ...
                // For other documents, just mark as uploaded
                setUploadedDocs({ ...uploadedDocs, [docId]: true });
                toast({
            ...
            setOcrErrors({ ...ocrErrors, [docId]: errorMessage });
            toast({
                title: "Upload error",
            ...
            setProcessingDocs({ ...processingDocs, [docId]: false });
        }
    };
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6 w-full">
                {requiredDocs.map((doc) => (
                    <Card key={doc.id} className={`h-full ${theme === "dark" ? "bg-gray-800 border-gray-700" : ""}`}>
                        ...
                        <CardContent>
                            <div
                                className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors cursor-pointer min-h-[180px] flex flex-col items-center justify-center ${theme === "dark"
                                    ? "border-blue-600 hover:bg-blue-900/20"
                                    : "border-blue-300 hover:bg-blue-50"
                                    } ${processingDocs[doc.id] ? "opacity-50 cursor-not-allowed" : ""}`}
                                onDragOver={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    if (!processingDocs[doc.id]) {
                                        e.currentTarget.classList.add(theme === "dark" ? "bg-blue-900/30" : "bg-blue-100");
                                    }
                                }}
                                onDragLeave={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    e.currentTarget.classList.remove(theme === "dark" ? "bg-blue-900/30" : "bg-blue-100");
                                }}
                                onDrop={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    ...
                                    if (processingDocs[doc.id]) return;

                                    const file = e.dataTransfer.files[0];
                                    if (file) {
                                        console.log('File dropped:', { docId: doc.id, fileName: file.name, fileSize: file.size, fileType: file.type });
                                ...
                                {processingDocs[doc.id] ? (
                                    <>
                                        ...
                                        <p className={`text-sm mb-3 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                                            {doc.id === "passport" ? "Processing document with OCR..." : "Uploading..."}
                                        </p>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <Input
                                            ...
                                            className="hidden"
                                            id={`file-${doc.id}`}
                                            accept={doc.id === "passport" ? "image/*,.pdf" : "*"}
                                            disabled={processingDocs[doc.id]}
                                            onChange={(e) => {
                                                const file = e.target.files?.[0];
                                                console.log('File input changed:', { docId: doc.id, file: file ? { name: file.name, size: file.size, type: file.type } : null });
                                                if (file) {
                                                    handleFileUpload(doc.id, file);
                                                } else {
                                        ...
                                        <Label htmlFor={`file-${doc.id}`} className="cursor-pointer">
                                            <Button variant="ghost" size="sm" className="mt-2" disabled={processingDocs[doc.id]}>
                                                Select File
                                            </Button>
                                        </Label>
                                    </>
                            ...
                            {ocrErrors[doc.id] && (
                                <div className={`mt-4 p-3 border rounded-lg ${theme === "dark"
                                    ...
                                    <div className="flex items-start gap-2">
                                        <AlertCircle className={`h-5 w-5 mt-0.5 ${theme === "dark" ? "text-red-400" : "text-red-600"}`} />
                                        <div className="flex-1">
                            ...
                            {uploadedDocs[doc.id] && !processingDocs[doc.id] && (
                                <div className={`mt-4 p-3 border rounded-lg ${theme === "dark"
                                    ...
                                    {doc.id === "passport" && ocrResults[doc.id] && (
                                        <div className={`mt-3 pt-3 border-t ${theme === "dark" ? "border-green-700" : "border-green-200"}`}>
                                            <p className={`text-xs font-semibold mb-2 ${theme === "dark" ? "text-green-300" : "text-green-700"}`}>
                                                Extracted Information:
                                            </p>
                                            <div className={`space-y-1.5 text-xs ${theme === "dark" ? "text-green-200" : "text-green-600"}`}>
                                                {ocrResults[doc.id].fullName && (
                                                    <div className="flex justify-between">
                                                        <span className="font-medium">Full Name:</span>
                                                        <span>{ocrResults[doc.id].fullName}</span>
                                                    </div>
                                                )}
                                                {ocrResults[doc.id].documentNumber && (
                                                    <div className="flex justify-between">
                                                        <span className="font-medium">Document Number:</span>
                                                        <span>{ocrResults[doc.id].documentNumber}</span>
                                                    </div>
                                                )}
                                                {ocrResults[doc.id].dateOfBirth && (
                                                    <div className="flex justify-between">
                                                        <span className="font-medium">Date of Birth:</span>
                                                        <span>{ocrResults[doc.id].dateOfBirth}</span>
                                                    </div>
                                                )}
                                                {ocrResults[doc.id].nationality && (
                                                    <div className="flex justify-between">
                                                        <span className="font-medium">Nationality:</span>
                                                        <span>{ocrResults[doc.id].nationality}</span>
                                                    </div>
                                                )}
                                                {ocrResults[doc.id].expiryDate && (
                                                    <div className="flex justify-between">
                                                        <span className="font-medium">Expiry Date:</span>
                                                        <span>{ocrResults[doc.id].expiryDate}</span>
                                                    </div>
                                                )}
                                            </div>

                                            {ocrResults[doc.id].rawText && (
                                                <details className={`mt-3 ${theme === "dark" ? "text-green-200" : "text-green-600"}`}>
                                                    <summary className="text-xs font-medium cursor-pointer hover:underline">
                                                        View Raw Text
                                                    </summary>
                                                    <pre className={`mt-2 p-2 rounded text-xs overflow-auto max-h-32 ${theme === "dark" ? "bg-green-900/50" : "bg-green-100"}`}>
                                                        {ocrResults[doc.id].rawText}
                                                    </pre>
                                                </details>
                                            )}
